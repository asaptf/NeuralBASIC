    ...
    monaco.languages.setMonarchTokensProvider("neuralbasic", {
      ...
      tokenizer: {
        root: [
          ...
          [
            /\b(dense|conv2d|flatten|pool|attention|transformer_block|transformer)\b/,
            "type",
          ],
          [
            /\b(activation|dataset|lr|epochs|l2|dropout|kernel|filters|heads|d_model|dff|mode|size|stride|global)\b(?==)/,
            "attribute.name",
          ],
          [/\b(sigmoid|relu|tanh|softmax|linear|max|avg)\b/, "string"],
          [/->|=>/, "operator"],
          [/\d+(\.\d+)?/, "number"],
          [/[{}]/, "delimiter.bracket"],
        ],
